...
(function ($) {
    $.fn.chart = function (settings, db) {
        ...
        function drawChart() {
            ...
            if (option.units)
                cdata['units'] = option.units;
            if (option.ymargin)
                cdata['ymargin'] = option.ymargin;
            if (option.hrule) {
                cdata.hrule = [];
                for (i = 0; i < option.hrule.length; i++) {
                    if (option.hrule[i].value)
                        cdata.hrule.push(option.hrule[i]);
            ...
            switch (option.type) {
                ...
                case "topn":
                    ...
                    for (k in sortedKeys) {
                        ...
                        if ("hidden" !== key)
                            labels[k] = $.map(key.split(option.sep), option.keyName);
                    }
